...
export function OrderConfirmationCard({
  ...
  className,
}: OrderConfirmationCardProps): React.JSX.Element {
  const details = [
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        aria-live="polite"
        className={cn(
          "w-full max-w-sm rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--card)] p-6 text-inherit shadow-[var(--shadow-soft)] backdrop-blur-[18px] sm:p-8",
          className,
        )}
      >
          ...
          <motion.div variants={itemVariants} className="w-full space-y-4 pt-4">
            {details.map((item, index) => (
              <div
                key={item.label}
                className={cn(
                  "flex items-center justify-between border-b border-[var(--line)] pb-4 text-sm text-[var(--ink-soft)]",
                  {
                    "border-none pb-0": index === details.length - 1,
                    "font-bold text-[var(--ink)]": item.isBold,
                  },
                )}
              >
